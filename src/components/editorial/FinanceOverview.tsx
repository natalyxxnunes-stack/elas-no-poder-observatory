...
export function FinanceByOffice({ snapshot }: { snapshot: TseFinanceSnapshot }) {
  ...
  return (
    <figure className="poster-frame overflow-hidden" aria-label="Presença das mulheres e participação na receita declarada, por cargo">
      <div className="flex gap-5 border-b border-rule px-5 py-3 font-mono text-[10px] text-muted-foreground">
      ...
      <div className="divide-y divide-rule">
        {rows.map((row) => row.office === "PRESIDENTE" ? (
          <div key={row.office} className="px-5 py-4">
            ...
            <p className="mt-2 font-mono text-[11px] text-muted-foreground">2 de 13 candidaturas com receita · 1,3% da receita</p>
          </div>
        ) : (
          <div key={row.office} className="grid gap-3 px-5 py-4 sm:grid-cols-[10rem_minmax(0,1fr)] sm:items-center">
            ...
            <div className="space-y-2">
              <div className="grid grid-cols-[minmax(0,1fr)_3.5rem] items-center gap-3"><div className="h-3 bg-secondary"><div className="h-full bg-plum" style={{ width: `${Math.min(row.presenceShare * 2, 100)}%` }} /></div><span className="font-mono text-[11px] text-ink">{formatPct(row.presenceShare)}</span></div>
              <div className="grid grid-cols-[minmax(0,1fr)_3.5rem] items-center gap-3"><div className="h-3 bg-secondary"><div className="h-full bg-coral" style={{ width: `${Math.min(row.revenueShare * 2, 100)}%` }} /></div><span className="font-mono text-[11px] text-ink">{formatPct(row.revenueShare)}</span></div>
            </div>
          </div>
        ))}
      </div>
      <figcaption className="border-t border-rule px-5 py-3 font-mono text-[10px] leading-relaxed text-muted-foreground">Presença entre candidaturas com receita e fatia da receita declarada do cargo. Régua de 0 a 50%. Contas de campanha em andamento, fotografia de {FINANCE_BASE_LABEL}. Receita, não despesa.</figcaption>
    </figure>
  );
}
  ...
  return (
    <div className="grid gap-5 md:grid-cols-2">
      {(["proporcional", "majoritario"] as const).map((universe) => {
        ...
        return <article key={universe} className="poster-frame overflow-hidden"><header className="border-b border-rule px-5 py-4"><h3 className="font-display text-xl text-ink">{UNIVERSE_LABEL[universe]}</h3><p className="mt-1 font-mono text-[10px] text-muted-foreground">Denominador: {formatBRLCompact(data.feminineRevenue)} de receita declarada por candidaturas de mulheres</p></header><dl className="divide-y divide-rule">{raceOrder.flatMap((race) => {
          const datum = data.feminineRevenueByRace[race];
          if (!datum) return [];
          return [<div key={race} className="grid grid-cols-[5rem_minmax(0,1fr)_8.5rem] items-center gap-3 px-5 py-3"><dt className="font-mono text-[10px] text-muted-foreground">{race.toLocaleLowerCase("pt-BR")}</dt><dd className="h-2 bg-secondary"><div className="h-full bg-plum" style={{ width: `${share(datum.value, data.feminineRevenue)}%` }} /></dd><dd className="text-right font-mono text-[10px] text-ink">{formatPct(share(datum.value, data.feminineRevenue))}<span className="block text-muted-foreground">{formatInt(datum.candidacies)} candidaturas</span>{universe === "proporcional" && <span className="block text-muted-foreground">mediana {formatBRLCompact(data.feminineMedianByRace[race] ?? 0)}</span>}</dd></div>];
...
export function FinanceByUf({ snapshot }: { snapshot: TseFinanceSnapshot }) {
  ...
  return <div className="grid gap-px border border-rule bg-rule sm:grid-cols-2 lg:grid-cols-3">{rows.map(([uf, datum]) => <article key={uf} className="bg-paper p-4"><div className="flex items-baseline justify-between"><h3 className="font-display text-xl text-ink">{uf}</h3><span className="font-mono text-xs text-plum">{formatPct(share(datum.feminine, datum.total))}</span></div><p className="mt-2 font-mono text-[11px] text-ink">{formatBRLCompact(datum.total)}</p><p className="mt-1 text-xs text-muted-foreground">Mulheres: {formatBRLCompact(datum.feminine)}</p></article>)}</div>;
}